...
from file_reader import load_annotation, load_protein
...
if __name__ == "__main__":
    with open("../../../config/basic/naive/naive_temporal_22.json") as fp:
        config = json.load(fp)
    ...
    with open(config["result"]["ltr"], 'w') as fp:
        json.dump(ltr_result, fp, indent=2)# indent=2, orient='index')
    with open(config["result"]["test"], 'w') as fp:
        json.dump(test_result, fp, indent=2)# indent=2, orient='index')
