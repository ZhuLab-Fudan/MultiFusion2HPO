...
import sys
sys.path.append("/share/inspurStorage/home1/zhaiwq/tmp/DeepText2HPO_pycharm_project")
...
def train(model, X_train, y_train, BATCH_SIZE, learning_rate, TOTAL_EPOCHS):
    ...
    for epoch in range(TOTAL_EPOCHS):
        ...
        # val_losses.append(val_loss)
        # print('Test set: Average loss: {:.4f}, Accuracy: ({:.3f}%), fmax: ({:.3f}%), threshold: ({.2f}%)\n'.format(
        #     val_loss, accuracy, fmax, threshold))
        # val_acc.append(accuracy)
...
if __name__ == "__main__":
    seed_torch()
    conf_path = "../../../config/basic/dnn/dnn_D2V.json"
