...
sys.path.append("/public/home/zhaiwq/modelused/DeepText2HPO_pycharm_project")
...
def task(dictionary):
    protein_name = list(dictionary.keys())[0]
    protein = dictionary[protein_name]
    if len(protein) <= 1500:
        return {protein_name: protein}
    uniprotein = {}
    key = sorted(protein, key=lambda k:protein[k])[::-1]
    for i in range(1500):
        uniprotein.update({key[i]: protein[key[i]]})
    return {protein_name: uniprotein}
...
def extract(res):
    ...
    with ThreadPoolExecutor() as pool:
        for protein in res:
            extractedValue = pool.submit(task, {protein: res[protein].copy()})
            res_list.append(extractedValue)
        pool.shutdown(wait=True)
    for uniprotein in res_list:
        try:
            sorted_annotation.update(uniprotein.result())
...
def multiProcessing_f_max(result, annotation, n_threshold=101):
    ...
    threshold = threshold_list[f_max_list.index(f_max_overall)]
...
if __name__ == "__main__":
    with open(path + "../../config/utils/evaluation/evaluation_all_full_esm2_geneexpression_biolinkbert_temporal_22_tfidfd2v.json") as fp:
        config = json.load(fp)
    ...
    for ns in ns_id + ["all"]:
        # load propagated HPO annotations of specified sub-ontology
        annotation_ns = load_annotation(config["ltr_annotation"], ontology, ns=ns)
        # transform it to double-layer dict, i.e.
        # { protein1: { hpo_term1: score1, hpo_term2: score2, ... }, ... }
        dict_annotation_ns = defaultdict(dict)
        for protein in annotation_ns:
