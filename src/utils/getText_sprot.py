...
batch = 0
accession = None
with open("/home/dengyj/uniprot/uniprot_sprot.dat") as fp:
    for line in fp:
        line = str(line)
        if line.startswith("AC"):
            if count >= 100000:
                ...
                print(batch)
                batch += 1
            accession = re.findall("\s+([A-Z0-9]+)?;", line)
            # print(accession)
            for protein in accession:
                text[protein] = []
            count += 1
            continue
        if line.startswith("RX"):
            try:
                for protein in accession:
                    text[protein].append(re.findall("PubMed=(\d+)?;", line)[0])
            except:
                # print(line)
                continue
for protein in protein_list:
...
with open("protein_text_sprot_201902.pkl", 'wb') as fp:
    pickle.dump(protein_text, fp)

with open("protein_file_sprot_201902.json", 'w') as fp:
